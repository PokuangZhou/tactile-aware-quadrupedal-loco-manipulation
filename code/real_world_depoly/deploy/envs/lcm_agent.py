...
class LCMAgent():
    def __init__(self, cfg, se, ac, command_profile):
        ...
        joint_names = [
            ...
            "RR_hip_joint", "RR_thigh_joint", "RR_calf_joint", "Z_Joint1", "Z_Joint2", "Z_Joint3", "Z_Joint4", "Z_Joint5", "Z_Joint6"]
        ...
        try:
            self.default_dof_pos_scale = np.array([self.cfg["init_state"]["default_hip_scales"], self.cfg["init_state"]["default_thigh_scales"], self.cfg["init_state"]["default_calf_scales"],
                                                   ...
                                                   self.cfg["init_state"]["default_hip_scales"], self.cfg["init_state"]["default_thigh_scales"], self.cfg["init_state"]["default_calf_scales"],
                                                   self.cfg["init_state"]["default_hip_scales"], self.cfg["init_state"]["default_thigh_scales"], self.cfg["init_state"]["default_calf_scales"],
                                                   self.cfg["init_state"]["Z_Joint1"],
                                                   self.cfg["init_state"]["Z_Joint2"],
                                                   self.cfg["init_state"]["Z_Joint3"],
                                                   self.cfg["init_state"]["Z_Joint4"],
                                                   self.cfg["init_state"]["Z_Joint5"],
                                                   self.cfg["init_state"]["Z_Joint6"]
                                                   ])
        except KeyError:
            self.default_dof_pos_scale = np.ones(18)
    ...
    def get_obs(self):
        ...
        self.arm_dof_pos = self.ac.get_arm_dof_pos()
        self.arm_dof_vel = self.ac.get_arm_dof_vel()

        obs_dof_pos = np.concatenate((self.dof_pos, self.arm_dof_pos), axis=-1)
        obs_dof_vel = np.concatenate((self.dof_vel, self.arm_dof_vel), axis=-1)
        ee_target = self.ac.get_ee_pos().copy()
        ee_target[0] = ee_target[0] - 0.3
        ee_target[2] = ee_target[2] - 0.09

        # ee_target = np.clip(ee_target, a_min=-0.1, a_max=0.1)

        ob = np.concatenate((self.gravity_vector.reshape(1, -1),
                             self.commands * self.commands_scale,
                             (obs_dof_pos - self.default_dof_pos).reshape(1, -1) * self.obs_scales["dof_pos"],
                             obs_dof_vel.reshape(1, -1) * self.obs_scales["dof_vel"],
                             ee_target.reshape(1, -1),
                             # ee_target.reshape(1, -1),
    ...
    def publish_action(self, action, hard_reset=False):
        ...
        self.joint_pos_target = self.joint_pos_target[..., :12]
        ...
        command_for_robot.kp = self.p_gains[:12]
        command_for_robot.kd = self.d_gains[:12]
        ...
        self.torques = (self.joint_pos_target - self.dof_pos) * self.p_gains[:12] + (self.joint_vel_target - self.dof_vel) * self.d_gains[:12]
        # Send the neural network action output to the C++ SDK via LCM.
        get_lcm().publish("pd_plustau_targets", command_for_robot.encode())

    def reset(self):
        self.actions = torch.zeros(12)
        self.time = time.time()
        self.timestep = 0
        self.ac.command = 'disable'
        return self.get_obs()
    ...
    def step(self, actions, hard_reset=False):
        ...
        if np.abs(self.commands[0, 0]) < 0.3 and np.abs(self.commands[0, 2]) < 0.6:
            self.gait_indices[:] = 0.
...
# The camera-related code below is disabled.
# --------------------------------------------------------------------
