...
class StateEstimator:
    def __init__(self, lc, use_cameras=False): # defaul use_cameras=True        
        ...
        self.dt_history = np.ones((self.smoothing_length, 1)) * 1e-3
...
# --------------------------------------------------------------
        ...
        self._dbg_imu_print_left = 10   # Print only the first 10 messages to avoid flooding the console.
        self._dbg_imu_last_print_t = 0.0
    ...
    def get_body_angular_vel(self):
        # dt_history shape: (N, 1) -> dt shape: (N,)
        dt = self.dt_history.squeeze(-1)
        # A valid dt is finite and greater than the threshold.
        valid = np.isfinite(dt) & (dt > 1e-6)

        # Compute ratios only at valid positions to avoid 0/0 or infinity.
        ratio = np.zeros_like(self.deuler_history, dtype=np.float64)  # (N, 3)
        if valid.any():
            ratio[valid] = self.deuler_history[valid] / dt[valid, None]
            mean_ratio = ratio[valid].mean(axis=0)
        else:
            mean_ratio = np.zeros(3, dtype=np.float64)

        # Sanitize mean_ratio in case deuler_history itself contains NaN values.
        mean_ratio = np.nan_to_num(mean_ratio, nan=0.0, posinf=0.0, neginf=0.0)

        # IIR smoothing.
        self.body_ang_vel = self.smoothing_ratio * mean_ratio + (1 - self.smoothing_ratio) * self.body_ang_vel

        # ===== Rate- and count-limited debug output =====
        # Optionally add these in __init__: self._dbg_ang_left = 20; self._dbg_ang_last_t = 0.0
        try:
            now = time.time()
            if getattr(self, "_dbg_ang_left", 0) > 0 and (now - getattr(self, "_dbg_ang_last_t", 0.0)) > 0.2:
                self._dbg_ang_left -= 1
                self._dbg_ang_last_t = now

                # print("\n[ANGVEL] ===== debug =====", flush=True)
                # print(f"🔍[ANGVEL] valid_count: {int(valid.sum())}/{len(valid)}", flush=True)
                # print(f"🔍[ANGVEL] dt min/max: {float(np.min(dt)):.6f} / {float(np.max(dt)):.6f}", flush=True)
                # print(f"🔍[ANGVEL] mean_ratio: {mean_ratio}", flush=True)
                # print(f"🔍[ANGVEL] body_ang_vel(out): {self.body_ang_vel}", flush=True)

                # To print only when an anomaly occurs, add:
                # if (not np.isfinite(self.body_ang_vel).all()) or (valid.sum() < len(valid)):
                #     print("...", flush=True)
        except Exception:
            pass
    ...
    def _imu_cb(self, channel, data):
        ...
        # ====== 1) Print the upstream message fields and common field values ======
        now = time.time()
        if self._dbg_imu_print_left > 0 and (now - self._dbg_imu_last_print_t) > 0.2:
            self._dbg_imu_last_print_t = now
            self._dbg_imu_print_left -= 1

            print("\n[IMU_CB] ===== RAW state_estimator_data =====", flush=True)
            # Some LCM types support __dict__ and others do not; try both approaches.
            try:
                keys = list(msg.__dict__.keys())
                print("🔍[IMU_CB] fields(__dict__):", keys, flush=True)
            except Exception:
                # Fallback: filter attributes returned by dir().
                keys = [k for k in dir(msg) if not k.startswith("_")]
                print("🔍[IMU_CB] fields(dir):", keys[:50], "...", flush=True)

            # Print common fields of interest when they are available.
            cand = [
                "utime", "rpy", "quat",
                "omega", "omegaBody", "gyro", "gyroBody",
                "vWorld", "vBody", "world_lin_vel", "world_ang_vel",
                "contact_estimate"
            ]
            for k in cand:
                if hasattr(msg, k):
                    v = getattr(msg, k)
                    print(f"[IMU_CB] {k}: {v}", flush=True)

        # ====== 2) Original logic: update orientation, contacts, and related state ======
        self.euler = np.array(msg.rpy)
        self.R = get_rotation_matrix_from_rpy(self.euler)
        self.contact_state = 1.0 * (np.array(msg.contact_estimate) > 200)

        # ====== 3) Print and validate dt to prevent zero from producing NaN ======
        dt = now - self.timuprev
        if not np.isfinite(dt) or dt <= 1e-6:
            print(f"[IMU_CB][WARN] dt abnormal: {dt}, forcing eps", flush=True)
            dt = 1e-3  # Fallback value.

        deuler = np.array(msg.rpy) - self.euler_prev

        # Also check for NaN values.
        if not np.isfinite(deuler).all():
            print(f"[IMU_CB][WARN] deuler has NaN/Inf: {deuler}", flush=True)

        self.deuler_history[self.buf_idx % self.smoothing_length, :] = deuler
...
# Should the camera-related functions below be removed?
# --------------------------------------------------
